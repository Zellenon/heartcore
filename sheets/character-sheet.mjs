...
export default class CharacterSheet extends ActorSheet {
    ...
    static get defaultOptions() {
        return foundry.utils.mergeObject(super.defaultOptions, {
            classes: ["heartcore", "sheet", "character"],
            template: "systems/heartcore/templates/character-sheet.html",
            tabs: [
                {
    ...
    async getData(options) {
        ...
        context.showSwingValueInput =
            context.data.system.swing.attributeId !== AttributeIdNoSwing;
    ...
    #populateAttributeStatusProperties(context) {
        for (let attribute of context.attributes) {
            if (attribute.system.cooldownType != 0 && attribute.system.status != 0) {
                attribute.cooldownString = AttributeCooldownStrings.get(
                    attribute.system.cooldownType,
                );
                if (attribute.cooldownString === undefined) {
                    throw new Error(
                        `Unexpected Attributecooldown ${attribute.system.cooldown} in attribute with ID ${attribute._id}`,
                    );
                }
                attribute.statusString = attribute.cooldownString;
            } else {
                attribute.statusString = AttributeStatusStrings.get(
                    attribute.system.status,
                );
                if (attribute.statusString === undefined) {
                    throw new Error(
                        `Unexpected AttributeStatus ${attribute.system.status} in attribute with ID ${attribute._id}`,
                    );
                }
            }
    ...
    async #onGiftAdd(event) {
        ...
        const sortValue =
            giftList.length > 0
                ? giftList[giftList.length - 1].sort + ListSortValueIncrement
                : 0;
    ...
    _onDrop(event) {
        ...
        this.#handleGiftDroppedOnList(
            droppedGiftId,
            giftDroppedUponId,
            giftListContainerHtml,
        );
    }
    ...
    #handleGiftDroppedOnList(
        ...
        const droppedGift = this.object.items.find(
            (item) => item._id === droppedGiftId,
        );
        if (!droppedGift) {
        ...
        const giftEquipStatusTo = Number(
            giftListContainerHtml.dataset["giftEquipStatus"],
        );
        ...
        if (destinationGiftList.length > 0) {
            ...
            if (giftDroppedUpon) {
                const movingToHigherList =
                    giftEquipStatusFrom !== giftEquipStatusTo &&
                    (giftEquipStatusTo == GiftEquipStatus.Primary ||
                        giftEquipStatusFrom == GiftEquipStatus.Unequipped);

                const movingUpWithinSameList =
                    giftEquipStatusFrom === giftEquipStatusTo &&
                    droppedGift.sort > giftDroppedUpon.sort;

                const giftDroppedUponListIndex = destinationGiftList.findIndex(
                    (item) => item._id === giftDroppedUpon._id,
                );
                targetListIndex =
                    movingToHigherList || movingUpWithinSameList
                        ? giftDroppedUponListIndex
                        : giftDroppedUponListIndex + 1;
            }
            ...
            if (targetListIndex == 0) {
                ...
                newSortValue =
                    destinationGiftList[destinationGiftList.length - 1].sort +
                    ListSortValueIncrement;
            } else {
                newSortValue =
                    (destinationGiftList[targetListIndex].sort +
                        destinationGiftList[targetListIndex - 1].sort) /
                    2;
            }
